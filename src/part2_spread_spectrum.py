...
def dsss_spread(bits, pn_chips):
    ...
    mapped = 1 - 2 * bits
    return np.repeat(mapped, len(pn_chips)) * np.tile(pn_chips, len(bits))
...
def despread_with_timing_offset(received_chips, pn_chips, max_offset):
    ...
    for offset in range(max_offset + 1):
        if offset >= len(received_chips):
            break
        truncated = received_chips[offset:]
        num_symbols = len(truncated) // len(pn_chips)
        if num_symbols <= 0:
            continue
        aligned = truncated[:num_symbols * len(pn_chips)]
        matrix = aligned.reshape(num_symbols, len(pn_chips))
        correlations = matrix @ pn_chips
    ...
    if best_bits is None:
        raise ValueError('no valid timing offset found for given input length')
    return best_bits
